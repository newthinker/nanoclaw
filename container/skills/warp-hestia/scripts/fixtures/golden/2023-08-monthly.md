...
- **排版**（见 `references/methodology.md` 的「排版纪律」）：每问先写一句**加粗结论**（≤30 字），三值以上的对比进小表，论证段 ≤150 字，**字段名放表格首列的标签里**（`住户存款 \`deposit_household_ytd\``）、**不要嵌在句子中间**；表格没覆盖的字段才写一行段末 `<sub>`；缺失与警示用 `⚠️` 单起一行。
