...
- 🔴 **`## 信号` / `## 趋势` / `## 社融增量结构` 三段都不发 check 行**。**不要按 `## ` 标题数去推
  check 行数**：机器区有 6 个 `## ` 而只有 2 条 check，「标题数 == check 数」这个判据装反了，
...
## 🔴 归档的字节 == 校验的字节（2026-09-16）

`verify.py` 校验的是**磁盘上**那份成稿；归档必须把**同一份**交给 Spool。

- skill 把成稿写进 `$Q/processing/<name>.note.md`（队列是可写挂载，宿主上即 `spool_content_root`）
- `selvage_call` 传 `content_path`，**不传 `content`**
- 归档后再对 vault 成品跑一次 `verify.py` 作事后闸

**为什么**：传 `content` 时 agent 是把文件内容**誊写**进工具调用参数的。2026-09-16 实测一次
真实损坏——8 处形近字替换，数字未受影响但封条不匹配。加图表之后机器区多了一千多个汉字，
誊写面积变大，缺口从「偶发」变成「必现」；**缺口一直在，图表只是把它照出来。**
